...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -target native -type event bpf bash.c -- -I../../headers
...
func main() {
	...
	up2, err := ex.Uretprobe(symbol, objs.UretprobeBashRetval, nil)
	if err != nil {
		log.Fatalf("creating uretprobe: %s", err)
	}
	defer up2.Close()
